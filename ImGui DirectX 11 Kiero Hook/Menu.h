...
void DrawMenu()
{
	...
	if (ImGui::Begin("[  IL2CPP  ]", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings))
	{
		...
		switch (tabb) {
		case 0:
			ImGui::Checkbox("Player Skeleton", &vars::PlayerSkeleton);
			ImGui::Checkbox("Player Chams", &vars::PlayerChams);
		...
		case 2:
			...
			if (ImGui::Checkbox("Infinite Ammo", &vars::infammo))
			{
				static uintptr_t InstructionAddy = (sdk::GameAssembly + 0x1A860C2);
				static char AmmoNop[] = { 0x90, 0x90, 0x90, 0x90 };
				static std::vector<unsigned char> OrigAmmo;
				if (vars::infammo)
				{
					DWORD old;
					VirtualProtect((LPVOID)InstructionAddy, 4, PAGE_EXECUTE_READWRITE, &old);
					OrigAmmo.resize(4);
					memcpy(OrigAmmo.data(), (LPVOID)InstructionAddy, 4);
					memcpy((LPVOID)InstructionAddy, AmmoNop, sizeof(AmmoNop));
					VirtualProtect((LPVOID)InstructionAddy, 4, old, &old);
				}
				else
				{
					DWORD old;
					VirtualProtect((LPVOID)InstructionAddy, 4, PAGE_EXECUTE_READWRITE, &old);
					memcpy((LPVOID)InstructionAddy, OrigAmmo.data(), OrigAmmo.size());
					VirtualProtect((LPVOID)InstructionAddy, 4, old, &old);
				}
			}

			ImGui::Checkbox("Weapon Mod", &vars::weaponmod);
			break;
		}
		ImGui::End();
	}
